...
void GSS_Chain::MatrixAppend(basketNode* basketCurr,int& level){
    valueCou+=1;
...
bool GSS_Chain::insert(string s1, string s2,int weight,double& insertMatrix)// s1 is the ID of the source node, s2 is the ID of the destination node, weight is the edge weight.
{
        ...
        if(!inserted){
            insertNum+=1;
            MatrixAppend(tempMainBasket,level);
...
void GSS_Chain::nodeSuccessorQuery(string s1, vector<ul> &neis){
    ul hash1 = mpq->MPQHash(s1.c_str(), Type);
    ul g1 = hash1 & fvalue[f];
    if(g1==0) g1+=1;
    ul h1 = (hash1>>f)%w;
    ...
    sucessorQuery(valueTree,0,poses,g,neis,0,0,false);
}

double GSS_Chain::neighborSimilarity(string s1, string s2){
    if(s1==s2)return 1;
    vector<ul> neis1, neis2;
    nodeSuccessorQuery(s1,neis1);
    nodeSuccessorQuery(s2,neis2);
