...
bool ComAuxo::insert(string s1, string s2,int weight,double& insertMatrix)// s1 is the ID of the source node, s2 is the ID of the destination node, weight is the edge weight.
{
        int res;
...
void ComAuxo::nodeSuccessorQuery(string s1, vector<ul> &neis){
    ...
    sucessorQuery(mainTree,0,poses,g,neis,0,0,false);
    sucessorQuery(deputyTree,0,poses,g,neis,0,0,false);
}
...
double ComAuxo::neighborSimilarity(string s1, string s2){
    if(s1==s2)return 1;
