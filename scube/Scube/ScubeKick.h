...
public:
    ...
    double nodeSimilarity(string s, string d) override;
    void nodeSucessorQuery(string s, vector<key_type>& neis);

private:
    int getMinIndex(uint16_t * a, int length);
    uint32_t recoverAddrSeed(fp_type fp, uint16_t idx, addr_type addr, uint32_t mod);
    double JaccardCoefficient(vector<key_type> &neis1, vector<key_type> &neis2);
...
double ScubeKick::JaccardCoefficient(vector<key_type> &neis1, vector<key_type> &neis2){
    if(neis1.size()==0||neis2.size()==0)return 0;
    sort(neis1.begin(),neis1.end());
    sort(neis2.begin(),neis2.end());
    double similarCou=0;
    int i=0,j=0;
    while(i<neis1.size()&&j<neis2.size()){
        if(neis1[i]==neis2[j]){
            similarCou+=1;
            i++;
            j++;
        }
        else if(neis1[i]<neis2[j]){
            i++;
        }
        else{
            j++;
    ...
    return similarCou/(neis1.size()+neis2.size()-similarCou);
}
...
/***********************************************/
void ScubeKick::nodeSucessorQuery(string v, vector<key_type>& neis){
            ...
            for (int j = 0; j < width; j++)
            {
                ...
                for (int k = 0; k < ROOM; k++)
                {
                    if((value[pos].fp_src[k] == fp_v) && ((value[pos].ext[k] >> 8) == i)) {
                        ...
                        uint32_t tmp_g = value[pos].fp_dst[k];
                        int tmp_s = value[pos].ext[k] & 0xff;
...
double ScubeKick::nodeSimilarity(string s, string d){
    if(s==d)return 1;
    vector<key_type> neis1, neis2;
    nodeSucessorQuery(s,neis1);
    nodeSucessorQuery(d,neis2);
    return JaccardCoefficient(neis1,neis2);
}
